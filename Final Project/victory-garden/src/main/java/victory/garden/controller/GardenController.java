...
public class GardenController {
	...
	public PlantData updatePlant(@PathVariable Long plantId, 
			@RequestBody PlantData plantData) {
		plantData.setPlantId(plantId);
		
		log.info("Modifying plant {}", plantData);
		return gardenService.savePlant(plantData);
	}
	...
	public PlantData retrievePlantById(
	...
	public Map<String, String> deletePlantById(
			@PathVariable Long plantId) {
		log.info("Deleting plant with ID={}", plantId);
		gardenService.deletePlantById(plantId);
		
		return Map.of("message", 
				"Deletion of plant with ID=" + plantId + " was successful.");
	}
	...
	public ScheduleData retrieveScheduleById(@PathVariable Long scheduleId) {
		log.info("Retrieving schedule with ID={}", scheduleId);
		return gardenService.retrieveScheduleById(scheduleId);
	}
	
	@GetMapping("/bed/{bedId}")
	public BedData retrieveBedById(@PathVariable Long bedId) {
		log.info("Retrieving bed with ID={}", bedId);
		return gardenService.retrieveBedById(bedId);
	}
